...
def simular_escenario_negocio(
    df_original, cambio_precio_porcentaje, presupuesto_marketing, nuevo_precio_unitario, costo_unitario
):
    """
    Calcula las ventas y ganancias reales basándose en el costo unitario.
    """
...
moneda_seleccionada = st.selectbox("¿En qué moneda quieres trabajar?", list(monedas.keys()))
moneda = monedas[moneda_seleccionada]
codigo_moneda = moneda["codigo"]
simbolo_moneda = moneda["simbolo"]
decimales_moneda = moneda["decimales"]
...
st.caption(f"Todos los valores de esta simulación se mostrarán en {codigo_moneda}.")
...
# ========================================================
# OPCIÓN A: CARGA, AUDITORÍA Y SIMULACIÓN DE CSV
# ========================================================
if modo_trabajo == "Tengo una base de datos (CSV)":
    ...
    # CREACIÓN DE LAS 4 PESTAÑAS PRINCIPALES
        ...
        try:
            ...
            if col_prod and not df_agrupado.empty:
                st.info(
                    ...
                    f"🚀 **¡Felicidades por tu estrella!** Tu producto rey (*{rey[col_prod]}*) es una verdadera máquina de tracción. Prohibido quedarse sin stock.\n\n"
                    f"💀 **Alerta de cuidados intensivos:** Ese *{hueso[col_prod]}* es tu producto hueso. Liquídalo urgente o arma un combo.\n\n"
                    f"🏷️ **Tu arma secreta:** Tienes el *{mas_barato[col_prod]}* como producto gancho. Úsalo en publicidad para atraer tráfico."
                )
            ...
            with col_op1:
                nuevo_precio = st.slider(
                    ...
                    min_value=0.0, max_value=tope_precio, value=float(precio_actual_calculado * 1.05), step=paso_slider
                )
            ...
            with col_op2:
                costo_unitario = st.slider(
                    ...
                    min_value=0.0, max_value=tope_precio, value=float(precio_actual_calculado * 0.40), step=paso_slider
                )
            ...
            try:
                ...
                (v_hist, g_hist, v_sim, g_sim, c_hist, c_sim) = simular_escenario_negocio(
                ...
                if g_sim > g_hist:
                    st.success(f"🚀 **¡Escenario Positivo!** Con esta configuración generas **{formatear_dinero(dif_ganancia)}** extra de ganancia neta pura.")
                else:
            ...
            st.error("⚠️ No se encontró el archivo 'train_2.csv' o 'train.csv'. Asegúrate de subirlo a GitHub.")
    ...
    with tab_asistido:
        ...
        st.write("Pronto activaremos la tabla interactiva para que rellenes tus costos manualmente.")
    ...
    # ---------------------------------------------------------
    # PESTAÑA 4: ESTRATEGIA DE MARKETING
    # ---------------------------------------------------------
    with tab_marketing:
        ...
        presupuesto_marketing = st.slider(
            ...
            min_value=0.0, max_value=tope_mkt, value=0.0, step=paso_mkt
        )
...
# ============================================================
# OPCIÓN B: USUARIO SIN CSV (MODO MANUAL)
# ============================================================
else:
    st.info("💡 Ingresa los datos generales de tu negocio y calculamos el resto.")
...
with st.expander("🔍 Ver datos utilizados en el análisis"):
    if df is not None and not df.empty:
        st.write("📊 **Muestra de la base de datos analizada:**")
        df_mostrar = df.head(10).copy()
        st.dataframe(df_mostrar, use_container_width=True, hide_index=True)
    else:
        st.write("📌 No hay datos activos.")

st.caption("Nota: las recomendaciones de marketing son estimaciones, no resultados garantizados.")
